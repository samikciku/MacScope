...
- The optional GPU helper must remain a separate `SMAppService` LaunchDaemon. Never run the MacScope GUI as root. Its XPC listener must enforce the main app's Developer ID identifier and team, and helper operations must remain fixed, read-only, time-bounded, and output-bounded. Ad-hoc builds must not enable helper registration.
